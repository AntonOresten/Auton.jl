...
function model_iteration(state::ConversationState; i=0)
    response = model_response_rules(state.model)(stdout) do streamcallback
        if isnothing(state.schema)
            PromptingTools.aigenerate(state.conversation; streamcallback, state.model, verbose=false)
        else
            PromptingTools.aigenerate(state.schema, state.conversation; streamcallback, state.model, verbose=false)
        end
    end
